from api.models import Referencia
from django.http import Http404
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
from rest_framework.permissions import IsAuthenticated
from api.serializers.persona.referenciaSerializer import ReferenciaSerializer

class referenciaList(APIView):

    permission_classes = (IsAuthenticated,)
    
    def get(self, request, format=None):
        referencia = Referencia.objects.filter(user=request.user)
        serializer = ReferenciaSerializer(referencia, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = ReferenciaSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class referenciaDetail(APIView):

    permission_classes = (IsAuthenticated,)

    def get_object(self, pk):
        try:
            return Referencia.objects.get(pk=pk)
        except Referencia.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        referencia = self.get_object(pk)
        serializer = ReferenciaSerializer(referencia)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        referencia = self.get_object(pk)
        serializer = ReferenciaSerializer(referencia, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
    def delete(self, request, pk, format=None):
        referencia = self.get_object(pk)
        referencia.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
